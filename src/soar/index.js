...
import { CustomSnackbar } from "../common/items/CustomSnackbar";
...
import AvFields from "./forms";
import DataImport from "./import";
import { Top } from "./Headers";
import Parameters from "./parameters";
import PeopleAvs from "./availability";
import SoarSettings from "./settings";
...
function Logo(props) {
  return (
    <Link
      to={`/${props.org}/schedules/${props.title}`}
...
export default function Soar() {
  ...
  const getData = useCallback(() => {
    getSchedule(org, load.sch, true, true).then((data) => {
      ...
      if (data.paramFields) {
        setParamFields(data.paramFields);
      }
      if (data.params) {
      ...
      let unpackedFuncs = {};
      Object.entries(data.funcs).forEach((item) => {
        unpackedFuncs[item[0]] = item[1]
          .split(";")
          .map((line) => line.split(","));
      });
      if (data.funcs) {
        setFuncs(unpackedFuncs);
      }
    });
  }, [load.sch, org]);
  ...
  async function save(location) {
    ...
    let tempFuncs = { ...funcs };
    Object.keys(tempFuncs).forEach((key) => {
      tempFuncs[key] = tempFuncs[key].join(";");
    });
    const data = {
      type: type,
      fields: fields,
  ...
  const tabs = {
    schedule: <Editor {...universalProps} />,
  ...
  return (
    <ThemeProvider theme={uTheme}>
      <AuthCheck>
        <AdminCheck
          org={org}
          helperText={"Sorry, this page is for authorized viewers only."}
        >
