...
import PopupForm from "../../common/layouts/PopupForm";
    ...
    <Paper variant={"outlined"} sx={{ height: "100%" }}>
      <Box padding={2}>
        <Box display={"flex"} flexDirection="row" mb={1}>
          <Typography sx={{ fontWeight: "bold" }}>{props.person}</Typography>
        </Box>
          ...
          <Grid container>
            {props.paramFields.map((item) => (
              <Grid item key={item.title} xs={6} sx={{ padding: 1 }}>
                <Typography
                  fontWeight={"bold"}
                  color={uTheme.palette.primary.main}
                >
                  {item.title}
                </Typography>
                <TextField
...
export default function Parameters(props) {
  ...
  const handleField = () => {
    if (title) {
      props.setParamFields(
        props.paramFields.concat({ title: title, type: type })
      );
      setTitle("");
      setOpen(false);
    }
  ...
  return (
    <div>
      ...
      <PopupForm open={open} setOpen={setOpen} title={"New Field"}>
        <TextField
          variant="outlined"
