...
function CodeEditor(props) {
  ...
  const handleChange = (index) => (event) => {
    if (ops[event.target.value]) {
      const line = [event.target.value].concat(
        [null, null, null].slice(0, ops[event.target.value].length)
      );
  ...
  return (
    <Paper variant="outlined" sx={{ margin: 1 }}>
        ...
        return (
          ...
          >
            ...
            <Autocomplete
              ...
              renderInput={(params) => (
                <TextField variant="standard" {...params} label="Operation" />
              )}
            />
      ...
      <Button onClick={handleAdd} sx={{ width: "100%" }}>
        <Add color={"secondary"} />
        <Typography sx={{ ml: 1 }}>Add a line</Typography>
      </Button>
    </Paper>
  );
}
...
function FunctionCard(props) {
  ...
  return (
    ...
    >
      ...
      >
        <AccordionSummary key={"top"} sx={{ px: 2, margin: 0 }}>
            ...
            >
              {menuItems.map((item) => (
                <MenuItem key={item} onClick={item[2]}>
                  {item[1]}
                  <Typography sx={{ ml: 1 }}>{item[0]}</Typography>
                </MenuItem>
              ))}
            </Menu>
          </Box>
  ...
  return (
    <Box sx={{ flex: 2, padding: 2 }}>
      ...
      >
        ...
        <Typography sx={{ ml: 2 }}>Add a function</Typography>
      </Button>
    </Box>
  );
}
