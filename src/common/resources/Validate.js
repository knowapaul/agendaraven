function email(input) {
  ...
  if (format.test(input)) {
    ...
    return [true, "Please input a valid email address"];
  }
}
...
function phone(input) {
  ...
  if (format.test(adapted)) {
    ...
    return [true, "Must contain 10 digits and no letters."];
  }
}
...
function title(input) {
  ...
  if (format.test(input)) {
    ...
    return [true, "Please input a name between 2 and 20 characters"];
  }
}
...
function password(input) {
  if (input.length > 8) {
    ...
    return [true, "Passwords must be longer than 8 characters"];
  }
}
...
function role(input) {
  if ((input.length > 2 && input.length < 20) || input.length === 0) {
    ...
    return [true, "Please input a role name between 2 and 20 characters"];
  }
}
...
function text(input) {
  if (/^.{2,30}$/.test(input)) {
    ...
    return [true, "Include 0 to 30 characters with no line breaks"];
  }
}
