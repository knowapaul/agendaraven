...
import { Box, Divider, Drawer, IconButton, List, ListItem, ListItemButton, ListItemIcon, ListItemText, Paper, Stack, ThemeProvider, Toolbar } from '@mui/material';
...
function Logo(props) {  
  return (
    <Link to={props.href} style={{textDecoration: 'none', color: uTheme.palette.primary.main}}>
      <Stack
      direction="row"
      spacing={.5} 
      margin={1}
...
function MenuItem(props) {
  ...
  return (
    ...
    sx={{
      backgroundColor: isSelected ? uTheme.palette.primary.main : uTheme.palette.background.default
    }}
    >
      <ListItemButton>
      ...
      <ListItemText primary={props.text} sx={{ color: isSelected ? uTheme.palette.background.default : uTheme.palette.primary.main}} />
      </ListItemButton>
    </ListItem>
  )
}
    ...
    <List>
      {props.items.map((item, n) => {
        ...
        return (
          ...
          :
          <Divider key={n} sx={{borderColor: uTheme.palette.primary.main, margin: 1}} />
        )
      })}
    </List>
  )
...
export default function DashModel(props) {
  ...
  return (
    <ThemeProvider theme={uTheme}>
      <AuthCheck>
        <Box sx={{ display: 'flex', flexDirection: 'row', height: smallScreen ? '100%' : '100vh', width: '100vw', position: 'static', top: 0, left: 0}}>
          <Box
            component="nav"
            sx={{ width: smallScreen ? undefined : drawerWidth, flexShrink: smallScreen ? undefined : 0 }}
            ...
            >
                ...
                <Toolbar disableGutters sx={{height: '100%' }} >
                  <IconButton
                    ...
                    edge="start"
                    onClick={handleDrawerToggle}
                    sx={{ mr: 2, display: smallScreen ? undefined : 'none', margin: 1 }}
                  >
              ...
              <Divider sx={{borderColor: uTheme.palette.primary.main, display: smallScreen ? 'none' : 'block'}} />
            </Paper>
