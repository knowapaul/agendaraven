...
import { MiniLoad } from "../../common/Loading";
import UserSearch from "../../common/UserSearch";
import { NavButton, SubNav } from "../../common/SubNav";
...
function Contact(props) {
  ...
  const index = people.includes(props.email)
    ? people.indexOf(props.email)
    : people.indexOf(props.email);
  people[index] = "Me";
...
function Write(props) {
  ...
  useEffect(() => {
    ...
    getSubscriptions(db, props.org, user.uid).then((subscription) => {
  ...
  return (
    <Box>
      ...
      <Paper square elevation={24} sx={{ height: "2px" }}>
        {sending ? <LinearProgress sx={{}} /> : ""}
      </Paper>
...
function NewChat(props) {
  ...
  const sendChatMessage = httpsCallable(
    props.firebase.functions,
    "sendChatMessage"
  );
