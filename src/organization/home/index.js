...
import { getMemo, getOrgFiles, setMemo } from "../../common/resources/Firebase";
    ...
    >
      <Box height={"162px"}>
        <FriendlyLoad
          ...
          alt={
            <img
              alt="default organization background"
              src="/abdfallback.png"
              width="288px"
              height="162px"
            />
      ...
      <Paper sx={{ mx: 2, my: 1, padding: 2 }}>
        {edit ? (
          ...
          <div>
            ...
            <Typography variant="body2">Written by {person}</Typography>
          ...
          <Box flex={0}>
            {edit ? (
              <Box display={"flex"} flexDirection={"row"}>
                <IconButton
                  onClick={() => {
                    setMemo(props.org, title, contents).then(() => {
                      setOpen(true);
                    });
                  }}
                >
    ...
    <Stack width={"100%"}>
      ...
      {files
        ? Object.keys(files).map((index) => (
            <FileItem key={files[index].name} item={files[index]} />
          ))
        : ""}
