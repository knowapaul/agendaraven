...
import Confirm from "../../common/items/Confirm";
...
export default function Archives(props) {
  ...
  useEffect(() => {
    getArchivedSchedules(props.org).then((tDat) => {
    ...
    <Paper variant="outlined" sx={{ margin: 2, padding: 2 }}>
      ...
      {Object.keys(data).length > 0 ? (
            ...
            .map((title) => (
              <ListItem key={title} disablePadding>
                ...
                >
                  ...
                  <ListItemText
                    primary={title}
                    sx={{ flex: 1, color: uTheme.palette.primary.main }}
                  />
                  <ListItemText
                    primary={data[title].type}
                    sx={{ flex: 1, color: uTheme.palette.primary.main }}
                  />
                  <ListItemText
                    primary={new Date(data[title].timestamp).toLocaleString(
                      "en-US",
        ...
        <Typography sx={{ margin: 2 }}>
          This folder is empty. In order to archive a schedule, go to its
          settings tab and click 'Archive'
        </Typography>
      )}
      <Confirm
        ...
        handleResult={() => {
          deleteSchedule(props.org, pendDelete)
            .then(() => {
              setResult("success");
              setSnackOpen(true);
            })
      ...
      <CustomSnackbar
        text={`Schedule ${
          result === "success" ? "successfully" : "could not be"
        } ${action}`}
        open={snackOpen}
