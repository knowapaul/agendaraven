...
import { NavButton, SubNav } from "../common/items/SubNav";
...
import { cTheme, uTheme, wTheme } from "../common/resources/Themes";
...
function ScheduleItem(props) {
  ...
  try {
    if (props.data.cats[props.field] === "person") {
      ...
      if (
        props.people[props.row[props.field]].uid ===
  ...
  return (
    ...
    >
      <Typography
        sx={{ margin: "auto", padding: "3px", textAlign: "center" }}
        variant={"body1"}
      >
        {displayText}
      </Typography>
...
const description = (data) =>
  (data.avDate
    ? `Availability Due: ${new Date(data.avDate).toLocaleString("en-US", {
        timeStyle: "short",
        dateStyle: "medium",
      })}\n`
    : "") +
  (data.timestamp
    ? `Last Edited: ${new Date(data.timestamp).toLocaleString("en-US", {
        timeStyle: "short",
        dateStyle: "medium",
      })}`
    : "");
    ...
    <div className="printHidden">
      <SubNav
        ...
        left={
          ...
          >
            <ArrowBack sx={{ mr: 1 }} />
            <Typography sx={{ display: { xs: "none", sm: "block" } }} noWrap>
              Schedules
            </Typography>
          </NavButton>
        }
...
function AvForm(props) {
  ...
  function save() {
    saveAvailability(props.org, props.title, data).then(() => {
      setSaved(true);
    });
  }
  ...
  return (
    <Box mt={2}>
      <div
        style={{
          display: props.avFields.length > 0 ? "flex" : "none",
          flexDirection: "row",
          padding: 8,
        }}
      >
        ...
        <Box>
          <Button variant={"contained"} onClick={save} sx={{ mr: 2 }}>
            <Save sx={{ mr: 1 }} />
            Save
          </Button>
        </Box>
      ...
      <Paper
        sx={{
          ...
          margin: 2,
          mt: 0,
          backgroundColor: props.overdue ? uTheme.palette.grey[300] : "none",
        }}
        variant="outlined"
      >
        {props.avFields.length > 0 ? (
          <div>
            <ThemeProvider theme={rTheme}>
              <Box
                display={props.overdue ? "block" : "none"}
                sx={{
              ...
              {props.avFields.map((field) => (
                <Box key={field.title}>
                  ...
                  <TextField
                    ...
                    onChange={(e) => {
                      let temp = { ...data };
                      temp[field.title] = e.target.value;
                      setData(temp);
                      setSaved(false);
                    }}
                    type={field.type}
          ...
          <Typography>
            No availability form has been posted for this schedule.
          </Typography>
        )}
      </Paper>
    </Box>
        ...
        {openBottom ? (
          <Paper
            ...
            sx={{
              ...
              backgroundColor: wTheme.palette.secondary.main,
              width: "100%",
            }}
          >
  ...
  useEffect(() => {
    getSchedule(load.org, load.sch)
      ...
      .then((nData) => {
        ...
        setView(
          nData?.avFields?.length > 0 && !tOverdue && !nData.contents[0]
            ? "availability"
            : "schedule"
        );
        setOverdue(tOverdue);
          ...
          <ErrorBoundary>
              ...
              <Box>
                ...
                >
                  <Tab
                    ...
                    label="Availability"
                    value="availability"
                    aria-controls="Select-Availability"
                  />
                  <Tab
                    sx={tProps}
                    label="Schedule"
                    value="schedule"
                    aria-controls="Select-Schedule"
                  />
                </Tabs>
              </Box>
            </ThemeProvider>
            ...
            {view === "availability" ? (
              ...
              <Typography sx={{ padding: 2 }}>
                We couldn't find a schedule to display yet. If that doesn't seem
                right, try contacting your organization's administrator.
              </Typography>
            )}
          </ErrorBoundary>
        </Box>
            ...
            <Paper square sx={{ padding: 2, mt: "1px" }}>
              <Typography variant="subtitle1">
                ...
                <AdminCheck
                  org={load.org}
                  helperText={
                    "The schedules below are all of the currently posted schedules in your organization."
                  }
                >
                  Below are all of your organization's schedules. Remember to
                  click the "Publish" button on your schedules before attempting
                  to view them here. Any schedule with a{" "}
                  <VisibilityOff fontSize="small" sx={{ pt: "3px" }} /> icon
                  before its name is hidden to everyone except administrators
