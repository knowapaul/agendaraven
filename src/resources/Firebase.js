...
export async function handleUpdatePassword(oldPassword, newPassword) {
  ...
  const cred = auth.currentUser.auth.EmailAuthProvider.credential(
...
export async function getRolesDoc(org, setDoc, setLoading, refresh) {
  ...
  if (await internalCheckAdmin(org)) {
    ...
    data = Object.keys(data).map((roleName) =>
      Object.assign(data[roleName], { roleKey: "#######" })
    );
  }
...
export async function getAvailability(org, schedule, setAvailability) {
  const data = await getData(
    org + "/agenda/availability/" + auth.currentUser.uid
  );
  if (data) {
...
export async function getSchedule(org, title, unpublished, refresh) {
  const data = await getData(
    org + `/agenda/${unpublished ? "unpublished" : "schedules"}/` + title,
    refresh
  );
  return data;
}
...
export function setMemo(org, title, contents) {
  ...
  return setDoc(doc(db, org + "/public"), {
    title: title,
    contents: contents,
...
export async function saveSchedule(org, title, data, location) {
  ...
  if (location === "archive") {
    ...
    batch.set(
      doc(db, org + `/agenda/archives/==index-file==`),
      { [title]: { type: data.type, timestamp: data.timestamp } },
      { merge: true }
    );
    ...
    if (location === "publish") {
      batch.set(doc(db, org + `/agenda/schedules/` + title), data, {
        merge: true,
      });
    }

    batch.set(doc(db, org + `/agenda/unpublished/` + title), data, {
      merge: true,
    });
