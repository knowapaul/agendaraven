  ...
  try {
    ...
    const response = await db.runTransaction(async (transaction) => {
      ...
      transaction.set(dataRef, {
        ...
        person: "AgendaRaven",
        contents:
          "You can use this widget to make announcements to your organization.",
      });
...
async function join(
  ...
  transaction.set(
    userDoc,
    { orgs: Object.assign(orgs, { [orgName]: true }) },
    { merge: true }
  );

  // Add to roles
  transaction.set(
    db.doc(orgName + `/public/users/${uid}`),
    {
      admin: role === "owner" || role.toLowerCase() === "admin",
      roles: [role],
    },
    { merge: true }
  );
  ...
  transaction.set(
    db.doc(orgName + "/private/docs/userData"),
    {
      [uid]: {
  ...
  try {
    ...
    const response = await db.runTransaction(async (transaction) => {
      const doc = await transaction.get(
        db.doc(data.orgName + "/private/docs/roleKeys")
      );

      if (!doc.data()) {
        throw new Error(
          "That organization does not exist. Please double-check your spelling and keep in mind that names are case sensitive."
        );
      }
      ...
      if (!role) {
        throw new Error(
          "Sorry, that join code is incorrect for this organization."
        );
      }
  ...
  .onRun(async (context) => {
    ...
    Object.keys(orgs).forEach(async (current) => {
      ...
      Object.keys(people).forEach(async (person) => {
        ...
        const recipient =
          "+1" +
          (await db.doc("users/" + person).get()).data().info.phonenumber;
        functions.logger.log("recipient", recipient);
        ...
        if (avs) {
          if (!avs[orgs[current]]) {
            functions.logger.log(
              "result",
              await sendTwilioMessage(client, twilioNumber, recipient, body)
            );
          }
        } else {
          functions.logger.log(
